...
const ProjectDetails = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const worksResponse = await userWorks(token);
        const tasksResponse = await userTasks(token);
  
        const storedWorks = JSON.parse(localStorage.getItem("works")) || [];
  
        setWorks([...worksResponse.data, ...storedWorks]);
  ...
  const getCompletedWorksFromLocalStorage = () => {
    const completedWorks = localStorage.getItem("works");
    return completedWorks ? JSON.parse(completedWorks) : [];
  };
  

  const setCompletedWorksToLocalStorage = (completedWorks) => {
    localStorage.setItem("completedWorks", JSON.stringify(completedWorks));
  };
  const setWorksToLocalStorage = (works) => {
    localStorage.setItem("works", JSON.stringify(works));
  };
  
  const markAsCompleted = (workId) => {
    setWorks((prevWorks) => {
      // Map works and mark the specified work as completed
      const updatedWorks = prevWorks.map((work) =>
        work.id === workId ? { ...work, status: "Completed" } : work
      );
  
      // Update local storage with the updated works
      setWorksToLocalStorage(updatedWorks);
  
      // Return the updated works to update the state
      return updatedWorks;
    }, () => {
      // Filter out the completed works from the in-progress works
      const inProgressWorks = works.filter(
        (work) => work.status.toLowerCase() === "working"
      );
  
      // Update the state for in-progress works
      setCompletedWorks(inProgressWorks);
  
      // Update local storage for completed works
      setCompletedWorksToLocalStorage(inProgressWorks);
    });
  };
...
<ItemWrapper>
  <Top>
    <Text>
      ...
      <Span>
        (
        {
          works.filter((item) => item.status.toLowerCase() === "working")
            .length
        }{" "}
        )
      </Span>
  ...
  <ResponsiveMasonry columnsCountBreakPoints={{ 350: 1, 750: 2, 900: 2 }}>
    <Masonry gutter="14px">
      {works
        .filter((item) => item.status.toLowerCase() === "working")
        .map((item) => (
  ...
  <Top>
    <Text>
      ...
      <Span>
        (
        {
          works.filter((item) => item.status.toLowerCase() === "completed")
            .length
        }{" "}
        )
      </Span>
          ...
          <div onClick={() => openWorkDetails(item)}>
            ...
            <Extra>
              ...
              <Table>
                ...
                {tasks.filter((item) => item.status === "Working" || item.status === "Completed")
